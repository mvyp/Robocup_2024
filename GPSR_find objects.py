...
class MoveRobot():
    ...
    def throw(self):#抛掷动作
        self.cartesian_pose_client(
            [0.34682925939559937, 0.4026565641164779663, 0.6062717616558075], [
...
if __name__ == '__main__':
    ...
    #选择四个点，先去房间的中心，即三角形的中心，随后三角形围绕
    current_goal_index = 0  # 初始化当前目标点索引为0
    rotate_count = 0  # 记录已经转过的次数
    while current_goal_index < len(goals):
        ...
        cpose = navi.move_group.get_current_pose().pose
        print(navi.target_list)

        while rotate_count < 4:  # 在当前点旋转4次，每次90度
            navi.rotate(90)  # 机器人旋转90度
            rospy.sleep(5)  # 每隔5秒
            if len(navi.target_list) != 0:#当检测到特定物品后
                relative_x = navi.target_position[0] - cpose.position.x
                relative_y = navi.target_position[1] - cpose.position.y

            # 计算机器人面向物体的角度
                target_angle = math.degrees(math.atan2(relative_y, relative_x))
              
                navi.goto([               
                    navi.target_list[0]-0.512062022090129, navi.target_list[1]-0.337210902202,
                    target_angle
                ])
                #前往检测到的物体的附近
                navi.main([                #如果找到特定物体，调用navi.main()函数执行抓取
                    navi.target_list[0]-0.0512062022090129, navi.target_list[1]-0.0237210902202,#分别是目标物体的xyz信息和旋转四元数，通过加减来微调 
                    navi.target_list[2]-0.00000129, 0.46103304624557495, 0.5499841570854187,0.44626110792160034,0.5346184372901917

                ])
            else:
                rotate_count += 1

            rotate_count = 0 # 重置旋转次数
